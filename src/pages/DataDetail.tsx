        ...
        <div className="md:flex">
          <div className="md:flex-shrink-0 p-6 md:p-0 md:w-1/3 flex justify-center items-center bg-slate-50">
            <img
              ...
              // DIUBAH: Pemanggilan fungsi melalui objek api
              src={api.utils.getMahasiswaFotoUrl(mahasiswa.foto) || 'https://via.placeholder.com/300'}
          ...
          <div className="p-6 md:p-8 flex-grow">
            ...
            <dl className="divide-y divide-slate-200">
              <div className={detailItemStyle}><dt className={detailLabelStyle}>Program Studi</dt><dd className={detailValueStyle}>{mahasiswa.prodi?.nama_prodi || 'Tidak ada data'}</dd></div>
              <div className={detailItemStyle}><dt className={detailLabelStyle}>Fakultas</dt><dd className={detailValueStyle}>{mahasiswa.prodi?.fakultas || 'Tidak ada data'}</dd></div>
              {mahasiswa.alamat && (
                <>
                  <div className={detailItemStyle}><dt className={detailLabelStyle}>Jalan</dt><dd className={detailValueStyle}>{mahasiswa.alamat.jalan}</dd></div>
                  <div className={detailItemStyle}><dt className={detailLabelStyle}>Kota</dt><dd className={detailValueStyle}>{mahasiswa.alamat.kota}</dd></div>
                  <div className={detailItemStyle}><dt className={detailLabelStyle}>Provinsi</dt><dd className={detailValueStyle}>{mahasiswa.alamat.provinsi}</dd></div>
                  <div className={detailItemStyle}><dt className={detailLabelStyle}>Kode Pos</dt><dd className={detailValueStyle}>{mahasiswa.alamat.kode_pos}</dd></div>
                </>
              )}
              {!mahasiswa.alamat && (<div className={detailItemStyle}><dt className={detailLabelStyle}>Alamat</dt><dd className={detailValueStyle}>Tidak ada data alamat.</dd></div>)}
            </dl>
            {isAdmin && (
              <div className="mt-8 flex space-x-3">
                <Link to={`/data-mahasiswa/edit/${mahasiswa.id}`} className="px-6 py-2 text-sm font-medium rounded-md text-white bg-yellow-500 hover:bg-yellow-600 focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:ring-offset-2">Edit Mahasiswa</Link>
                <button onClick={handleDelete} disabled={deleteMutation.isPending} className="px-6 py-2 text-sm font-medium rounded-md text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 disabled:bg-slate-400">
